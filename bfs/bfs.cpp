...
#pragma omp atomic capture
            { offset = new_frontier->count; new_frontier->count += (int)local.size(); }
...
void bfs_hybrid(Graph graph, solution* sol) 
{
    ...
    if (bu_threshold < 1) bu_threshold = 1;
    if (td_threshold < 1) td_threshold = 1;
    bool use_bottom_up = false;

    while (frontier->count != 0) {
        if (use_bottom_up) {
            if (frontier->count <= td_threshold)
                use_bottom_up = false;
        } else {
            if (frontier->count >= bu_threshold)
                use_bottom_up = true;
        }
